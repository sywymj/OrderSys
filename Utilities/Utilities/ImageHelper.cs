...
namespace JSNet.Utilities
{
    ...
    public class ImagerHelper
    {
        ...
        /// <param name="width">缩略图宽度</param>
        /// <param name="height">缩略图高度</param>
        /// <param name="mode">生成缩略图的方式,0:HORW指定高宽缩放(不补白),1:HW,2:W,3:H,4:Cut</param>    
        public static void MakeThumbnailImage(string fileName, string newFileName, int width, int height, int mode)
        {
            byte[] imageBytes = File.ReadAllBytes(fileName);
        ...
        /// <returns></returns>  
        public static bool readPictureDegree(string path)
        {
            try
            {
                        ...
                        if (prop.Id == 0x112)
                        {
                            switch (prop.Value[0])
                            {
                                case 2:
                                    ...
                                    //RotateImg2(img, 0).Save(path);
                                    break;
                                case 3:
                                    img.RotateFlip(RotateFlipType.Rotate180FlipNone);//right-top  
                                    //RotateImg2(img,180).Save(path);
                                    break;
                                case 4:
