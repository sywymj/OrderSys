...
namespace CodeEngine.Framework.QueryBuilder
{
    ...
    public static class PagingBuilder
    {
        ...
        /// <param name="_orderField">排序字段，多个则用“,”隔开，不可空，必需有排序字段</param>
        ...
        public static string CreatePagingSql(int _recordCount, int _pageSize, int _pageIndex, string _safeSql, string _orderField)
        {
            ...
            for (int i = 0; i < arrStrOrders.Length; i++)
            {
                ...
                int index = arrStrOrders[i].IndexOf(" ASC") > 0 ? arrStrOrders[i].IndexOf(" ASC") : arrStrOrders[i].IndexOf(" DESC"); //判断是否有升降标识
                if (index > 0)
                {
                    //替换升降标识，分页所需
            ...
            if (_pageIndex == 1)
            {
                ...
                sbSql.AppendFormat(" {0} ", sbOriginalOrder.ToString());
            }
            //最后一页时，减少一个TOP
            else if (_pageIndex == pageCount)
            {
                ...
                sbSql.AppendFormat(" FROM ({0}) AS T ", _safeSql);
                sbSql.AppendFormat(" {0} ", sbReverseOrder.ToString());
                sbSql.Append(" ) AS T ");
                sbSql.AppendFormat(" {0} ", sbOriginalOrder.ToString());
            }
            //前半页数时的分页
